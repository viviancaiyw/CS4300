# Reference: 
# https://github.com/woctezuma/download-steam-reviews/blob/master/steamreviews/download_reviews.py
...
# Function to call.
def get_reviews(app_id, verbose=False):
	...
	review_dict, query_count = download_reviews_for_app_id(app_id, 
		...
		verbose=verbose)
	print("result length:", len(review_dict['reviews']))
	return review_dict
...
def download_reviews_for_app_id(app_id,
	...
	# Changed
	counter = 0
	while (num_reviews is None) or len(new_review_ids)<NUM_REVIEWS:
#     (offset < num_reviews):
		...
		if success_flag and delta_reviews > 0:
			...
			if check_review_timestamp:
				...
				checked_reviews = list(filter(lambda x: x[timestamp_str_field] > timestamp_threshold and
					x['written_during_early_access']==False and x['author']['playtime_forever']>60,
											  downloaded_reviews))
