...
'''
def get_word_to_tags(tags):
	words = list(map(lambda x: x.split(), tags))
	words = list(itertools.chain(*tags))

	word_to_tags = {word:list() for word in words}
	for word in words:
		for tag in tags:
			if word in tag:
				word_to_tags.get(word).append(tag)
	return word_to_tags
...
'''
def get_keyword_match_wordvector(tag_words, common_keywords_keyphrases):
	addedword_to_originalwords = dict()

	# Get syns for each word in tags
	word_to_syns = dict()
	for word in tag_words:
		syns = word_to_syns.get(word, list())
		syns.extend(get_syns(word))
	...
	for syns in word_to_syns.values():
...
'''
def change_addedword_to_originalword(intersect, addedword_to_originalwords):
	res = set()
...
'''
def match_games_using_keywords(tags, appid_to_vec, common_keywords_keyphrases):

	def get_sim_res(d1, d2, norm1, norm2, addedword_to_originalwords):
		(score, intersect) = cos_sim(d1, d2, norm1, norm2)
		intersect = change_addedword_to_originalword(intersect, addedword_to_originalwords)
		return (score, intersect)
	...
	vector, norm, addedword_to_originalwords = get_keyword_match_wordvector(tag_words, common_keywords_keyphrases)
	...
	for word in words_only:
		addedword_to_originalwords[word] = [word]

	res = list(map(lambda x: 
						(x['app_id'], get_sim_res(vector, x['vector'], norm, x['norm'], addedword_to_originalwords)), 
					appid_to_vec.values()))
...
'''
def match_games_using_keyphrases(tags, appid_to_vec, common_keywords_keyphrases, word_to_synphrases, inv_keywords_phrases):
	weight = dict()
	...
	word_to_tags = get_word_to_tags(words)
...
return [(app_id, (score, tag_matchs))] in ranking order.
'''
def merge_keyword_keyphrase_match_results(keyword_matchs, keyphrase_matchs):
	# Assign new score.
...
'''
Get tags match results.

return [(app_id, (score, tag_matchs))]
'''
def match_tags(input_tags, common_keywords_keyphrases, word_to_synphrases, inv_keywords_phrases, appid_to_vec):
	tags = clean_tags(input_tags)

	keyword_matchs = match_games_using_keywords(tags, appid_to_vec, common_keywords_keyphrases)
	keyphrase_matchs = match_games_using_keyphrases(tags, appid_to_vec, common_keywords_keyphrases, word_to_synphrases, inv_keywords_phrases)

	res = merge_keyword_keyphrase_match_results(keyword_matchs, keyphrase_matchs)
...
if __name__ == "__main__":
	...
	res = match_tags(input_tags, common_keywords_keyphrases, word_to_synphrases, inv_keywords_phrases, appid_to_vec)
