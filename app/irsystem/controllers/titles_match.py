...
def get_matched_game_ranking_info_for_movie(movie_id: str) -> Dict[str, int]:
    """
...
def get_matched_game_ranking_info_for_game(app_id: str) -> Dict[str, int]:
    """
    Query the corresponding match ranking from DB.
    """
    query_res = Game.query.filter_by(app_id=app_id).all()
    if query_res:
        return eval(query_res[0].games_title_match)
    return dict()


def get_top_games_from_title_for_movie(mov_tmt_path: str,
    ...
    if not mov_tmt_path or not candidate_games:
        return []
    ranking_info = get_matched_game_ranking_info_for_movie(
        movie_id=mov_tmt_path)
    res = []
...
def get_top_games_from_title_for_game(app_id: str, candidate_games: List[str],
                                      threshold=0.5) -> List[Dict]:
    """
    Given a game's id `app_id`, return a map of all top similar games
    i.e. similar score between titles > `threshold` who
    appears in `candidate_games`
    """
    if not app_id or not candidate_games:
        return []
    ranking_info = get_matched_game_ranking_info_for_game(app_id=app_id)
    res = []
    for app_id, score in ranking_info.items():
        if score >= threshold and app_id in candidate_games:
            game_obj: Game = Game.query.filter_by(app_id=app_id).all()[0]
