...
from app.irsystem.controllers import MOVIE_INFO, MOVIE_GAME_TITLE_SIMILARITY, MOVIE_FILTERED_TITLE_MAP
...
#     top_match = sim_dict.get(filtered_mov_name)
#     if not top_match:
#         return []
#
...
#     return res
#
# def movie_df_to_json():
#     mov_4_col_path = os.path.join(DATA_DIR, 'movie_data','movie_title_filtered_df_3.csv')
#
#
#
...
#     matched_rows = movie_filtered_df[
...
def get_top_games_from_title_v2(mov_tmt_path):
    # movie_raw_name = MOVIE_INFO[mov_tmt_path]['name']
    # movie_filtered_name = MOVIE_FILTERED_TITLE_MAP[mov_tmt_path]
    ranking_info = MOVIE_GAME_TITLE_SIMILARITY[mov_tmt_path]
    res = []
    for app_id, score in ranking_info.items():
        res.append((app_id, score))
    return str(res)
...
#     print(get_top_games_from_title_v2('The Lego Movie'))
